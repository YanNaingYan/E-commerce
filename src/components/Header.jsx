...
const Header = () => {
  const { selectedProducts, toggleCartDrawer } = useContext(DataContext);
    ...
    <header className="border-b-2 z-40 fixed w-full bg-white border-neutral-600">
      <Container>
        <div className="flex justify-between items-center py-3">
          <div className="brand">
            <h1 className="font-heading font-bold text-2xl">Shopping</h1>
            <p className="text-neutral-500">E-commerce App</p>
          </div>
          <div className="control-btn flex gap-2">
            <div
              id="searchBar"
              className={`border border-neutral-600 h-12 w-60 flex gap-3 px-2 items-center  ${
                !open && "opacity-0 -translate-y-full"
              }`}
            >
              <input
                ref={inputRef}
